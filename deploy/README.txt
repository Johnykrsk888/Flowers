...
Учётные данные MariaDB для каталога берутся так: (1) Secrets CATALOG_MYSQL_USER / CATALOG_MYSQL_PASSWORD, или (2) MYSQL_USER / MYSQL_PASSWORD, или (3) файл на VPS `/etc/flowers/mysql-dotenv.txt` (его создаёт vps-mysql-phpmyadmin.sh) — отдельные секреты не обязательны.
